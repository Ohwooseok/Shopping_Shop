package com.likelion.lionShop.Security.utils;
...
public class JwtUtil {
    ...
    public String tokenProvider(CustomUserDetails customUserDetails, Instant expiration) {
        ...
        return Jwts.builder()
                ...
                .claim(IS_STAFF, customUserDetails.getStaff()) //권한 추가
    ...
    private Claims getClaims(String token) {
        log.info("[ JwtUtil ] 토큰에서 Claim을 추출합니다.");
        try {
            return Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload();
        } catch (UnsupportedJwtException | MalformedJwtException | IllegalArgumentException e) {
            throw new SecurityException("올바르지 않은 토큰입니다");
        } catch (SignatureException e) {
            throw new SecurityException("토큰이 위조되거나 손상되었습니다.");
        }
    }
